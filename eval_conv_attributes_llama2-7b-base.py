...
def save_to_db(user_responses, start_time, elapsed_seconds):
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS rater_evaluations_llama_2_7b_chat (
            ...
            insert_query = """
                INSERT INTO rater_evaluations_llama_2_7b_chat
...
def main():
    ...
    if "df_local" not in st.session_state:
        try:
            st.session_state["df_local"] = pd.read_csv("llama-2-7b-chat-conversations.csv",    encoding="latin1", on_bad_lines="skip")
